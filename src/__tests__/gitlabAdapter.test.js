...
const mapsUsers = () => {
  const users = Adapter.mapUsers(usersMock);
...
const mapsCommits = () => {
  const commits = Adapter.mapCommits(commitsMock);
  const expected = {
    author: "test user2",
...
const mapsProjects = () => {
  const projects = Adapter.mapProjects(projectsMock);
...
const mapsComments = () => {
  const users = Adapter.mapUsers(usersMock);
  const comments = Adapter.mapCommentsToUsers(commentMock, users);
