...
function* iterateBranch(id, branch, total, Api) {
  ...
  const cd = yield call(Api.fetchComments, id, branch, page);
  ...
  while (page <= totalPages) {
    ...
    calling = yield call(Api.fetchComments, id, branch, page);
    calling.data.forEach(c => {});
    const ended = new Date().getTime();
    comments =
...
export const CommentsSagas = [
  ...
  takeLatest("USERS_UPDATED", remapUsersToComments)
];
